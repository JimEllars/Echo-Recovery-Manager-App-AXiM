...
// .wrangler/tmp/bundle-gscdrP/checked-fetch.js
...
async function handleIngress(request, env) {
  ...
  if (missingVars.length > 0) {
    const details = missingVars.map((v) => `Missing environment variable: ${v}`).join(", ");
    return new Response(
  ...
  if (contentLength && parseInt(contentLength, 10) > MAX_PAYLOAD_SIZE) {
    return new Response(
      JSON.stringify({
        error: "Payload too large. Maximum supported ingress payload size is 256 KB."
      }),
      {
        status: 413,
        headers: {
...
async function handleReplay(request, env, ctx, operator_id) {
  ...
  if (recordIds.length > 50) {
    return new Response(
      JSON.stringify({
        error: "Batch limit exceeded. Please send a maximum of 50 records per request to prevent execution timeouts."
      }),
      {
        status: 400,
        headers: {
  ...
  try {
    ...
    if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
      ...
      try {
        ...
        throw new Error(
          `Failed to parse JSON substring: ${innerError.message}`
        );
      }
    }
...
async function handleTriage(request, env, operator_id) {
  ...
  if (!recordId) {
    return new Response(
      JSON.stringify({ error: "Bad Request: recordId is required" }),
      {
        status: 400,
        headers: {
          "Content-Type": "application/json",
...
Original Payload: ${JSON.stringify(originalPayload)}`
      ...
      if (!aiResponse.ok) {
        ...
        throw new Error(
          `AI Proxy responded with ${aiResponse.status}: ${aiErr}`
        );
      }
    ...
    if (!updateResponse.ok) {
      ...
      return new Response(
        JSON.stringify({
          error: "Failed to update record with patch",
          details: updateErr
        }),
        {
          status: 500,
    ...
    return new Response(
      JSON.stringify({ success: true, patched: true, patch: proposedPatch }),
      {
        status: 200,
        headers: {
          "Content-Type": "application/json",
...
// .wrangler/tmp/bundle-gscdrP/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-gscdrP/middleware-loader.entry.ts
