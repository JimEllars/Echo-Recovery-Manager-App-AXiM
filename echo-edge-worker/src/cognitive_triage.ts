  ...
  try {
    ...
    if (match && match[1]) {
      try {
        ...
        throw new Error(
          `Failed to parse extracted JSON block: ${(innerError as Error).message}`,
        );
      }
    }
    ...
    if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
      ...
      try {
        ...
        throw new Error(
          `Failed to parse JSON substring: ${(innerError as Error).message}`,
        );
      }
    }
...
export async function handleTriage(
  ...
  try {
    ...
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an autonomous data recovery system. Analyze the provided original JSON payload and the error reason. Return ONLY the corrected JSON object that fixes the error. Do not include any explanations, markdown formatting, or preamble. Return raw valid JSON.",
          },
          {
            role: "user",
      ...
      const llmText =
        aiData.choices?.[0]?.message?.content || aiData.response || "";
